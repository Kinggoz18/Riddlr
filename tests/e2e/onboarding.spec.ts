...
async function expectVisibleCopy(page: Page, pattern: string | RegExp) {
...
test("first-run onboarding is four steps with crypto supported and other domains coming soon @a11y", async ({
  ...
  await expect(page.getByRole("button", { name: "Skip for now" })).toBeVisible();
  await expect(page.getByText(/checks that the model answers before saving/i)).toBeVisible();
  await page.getByRole("button", { name: "Skip for now" }).click();
  ...
  await page.getByRole("checkbox", { name: /Equities/ }).check();
  await expect(page.getByRole("checkbox", { name: /Equities/ })).toBeChecked();
  await expect(page.getByRole("checkbox", { name: /Crypto/ })).toBeChecked();
  ...
  await expect(
    page.getByText(/Riddlr Intelligence Agent · \d+ prompt tokens recorded/),
  ).toBeVisible();
  await expect(page.getByText(/Riddlr Intelligence Agent · \d+ assets/)).toBeVisible();
});
...
test("dashboard surfaces, settings, health, and responsive layout @a11y", async ({ page }) => {
  ...
  await expect(page.locator("html")).toHaveAttribute("data-theme", "dark");
  await primaryNav(page).getByRole("link", { name: "Agents", exact: true }).click();
  ...
  await primaryNav(page).getByRole("link", { name: "Agents", exact: true }).click();
  await expect(page.getByRole("heading", { name: "Agents" })).toBeVisible();
  await expect(page.getByRole("link", { name: "Riddlr Intelligence Agent" })).toBeVisible();
  await primaryNav(page).getByRole("link", { name: "Sources", exact: true }).click();
  ...
  await expectVisibleCopy(page, /MESSAGE_CONTENT/);
  await expectVisibleCopy(page, /not guild message-search archive/i);
  await page.getByRole("link", { name: "Add source" }).click();
  ...
  await expectVisibleCopy(page, /If-None-Match/);
  await page.getByRole("link", { name: "Add source" }).click();
  ...
  await expectVisibleCopy(page, /Personal, non-commercial/i);
  await page.getByRole("link", { name: "Add source" }).click();
  ...
  await expectVisibleCopy(page, /No API key/i);
  await page.getByRole("link", { name: "Add source" }).click();
  ...
  await expectVisibleCopy(page, /annualised APR/i);
  await page.getByRole("link", { name: "Add source" }).click();
  ...
  await expectVisibleCopy(page, /KXCPI/);
  await page.getByRole("link", { name: "Add source" }).click();
  ...
  await expectVisibleCopy(page, /Never paste a seed phrase or private key/i);
  await expectVisibleCopy(page, /\/hooks\/\*/);
  await page.getByRole("link", { name: "Add source" }).click();
  ...
  await expectVisibleCopy(page, "Recent search only");
  await expect(page.getByLabel("Monthly read budget")).toBeVisible();
  await expectVisibleCopy(page, /Named principals/i);
  await primaryNav(page).getByRole("link", { name: "Portfolios", exact: true }).click();
  await expectVisibleCopy(page, /Never enter a seed phrase/i);
  await primaryNav(page).getByRole("link", { name: "Signals", exact: true }).click();
  ...
  await primaryNav(page).getByRole("link", { name: "Settings", exact: true }).click();
  await expect(
    page.getByRole("region", { name: "Configuration status" }).getByText("Missing"),
  ).toBeVisible();
  await expect(
    page.getByRole("region", { name: "Configuration status" }).getByText(/Key \d+/),
  ).toBeVisible();
  ...
  await primaryNav(page).getByRole("link", { name: "Health", exact: true }).click();
...
test("watchlists, notifications, 404, and tablet layout @a11y", async ({ page }) => {
  ...
  await primaryNav(page).getByRole("link", { name: "Scorecard", exact: true }).click();
  await expect(page.getByRole("heading", { name: "Scorecard" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "No scored signals" })).toBeVisible();
  await primaryNav(page).getByRole("link", { name: "Events", exact: true }).click();
  ...
  await expect(page.getByRole("img", { name: /Spot price/ }).first()).toBeVisible();
  await primaryNav(page).getByRole("link", { name: "Notifications", exact: true }).click();
