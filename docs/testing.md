...
taxonomy mapping, quantitative claim contracts, event lifecycle and outcomes, typed signal policies, and coming-soon rejection. Architecture tests keep generic packages from importing
...
event lifecycle join/reopen/merge, outcome recording, the scorecard API, and typed signal persistence.
